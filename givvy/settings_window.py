"""Settings: everything that would clutter the main window.

  Emulators  cores / RAM / graphics per emulator (the AVD's config.ini), plus CPU
             pinning and priority, which are what actually dedicate resources
  Bot        the handful of numbers worth changing without a text editor
"""
from __future__ import annotations

import logging
import time
import os
import threading
import tkinter as tk
from tkinter import messagebox, ttk

from . import avd_settings as A

log = logging.getLogger(__name__)

GPU_LABELS = {"default": "default  (emulator decides)", "host": "host  (graphics card)", "software": "software  (CPU)"}
PRIORITY_LABELS = {"normal": "normal", "below_normal": "below normal", "idle": "idle", "above_normal": "above normal"}
RAM_CHOICES = ("2048", "3072", "4096", "6144", "8192")
HOP_LABELS = {"live": "live", "log": "log only (never moves)", "off": "off"}     # [hop] mode, the crowd hop's switch


def _key(labels: dict, shown: str) -> str:
    return next((k for k, v in labels.items() if v == shown), shown)


def suggest_pinning(n_emulators: int, cores_each: list[int], logical_cpus: int) -> list[str]:
    """Hand out separate core ranges from the TOP of the CPU down, so the low cores,
    which Windows and games favour, stay free. Blank when it cannot fit."""
    out, top = [], logical_cpus
    for want in cores_each[:n_emulators]:
        lo = top - want
        if lo < 0:
            out.append("")
            continue
        out.append(f"{lo}-{top - 1}" if want > 1 else f"{lo}")
        top = lo
    return out


class SettingsWindow(tk.Toplevel):
    def __init__(self, app, tab: str = ""):
        super().__init__(app)
        self.app, self.engine = app, app.engine
        self.title("Whatnot Givvy - Settings")
        self.transient(app)
        self.resizable(False, False)
        nb = ttk.Notebook(self)
        nb.pack(fill="both", expand=True, padx=10, pady=10)
        self.emu_tab = ttk.Frame(nb, padding=12); nb.add(self.emu_tab, text="Emulators")
        self.bot_tab = ttk.Frame(nb, padding=12); nb.add(self.bot_tab, text="Bot")
        self.black_tab = ttk.Frame(nb, padding=12); nb.add(self.black_tab, text="Blacklist")
        self.results_tab = ttk.Frame(nb, padding=12); nb.add(self.results_tab, text="Results")
        from .check_tab import EmulatorCheckTab
        self.check_tab = EmulatorCheckTab(nb, app); nb.add(self.check_tab, text="Emulator check")
        self.nb = nb
        if tab == "check":
            nb.select(self.check_tab)
        self._build_emulators()
        self._build_bot()
        self._build_blacklist()
        self._build_results()
        self._build_bar(nb)
        self.bind("<Escape>", lambda _e: self.destroy())

    def _build_bar(self, nb) -> None:
        """The bottom bar: the version, 'Check for updates' (how the friend's install gets new versions), Apply and
        Close. Packed at the bottom ahead of the notebook, so a short screen squeezes the tabs and never the bar: with
        the trial report the window asked for 1106 x 798 px (+ its title bar) and cannot be resized, more than a
        1366 x 768 laptop, or 1920 x 1080 at 150%, has room for (measured 2026-09-24, 100% scaling, 3 accounts)."""
        bar = ttk.Frame(self)
        bar.pack(side="bottom", fill="x", padx=10, pady=(0, 10), before=nb)
        from .version import build_info
        ttk.Label(bar, text=build_info(), style="Faint.TLabel").pack(side="left")
        self.update_btn = ttk.Button(bar, text="Check for updates", command=self._check_updates)
        self.update_btn.pack(side="left", padx=12)
        self.update_lbl = ttk.Label(bar, text="", style="Muted.TLabel", wraplength=520, justify="left")
        self.update_lbl.pack(side="left")
        ttk.Button(bar, text="Close", command=self.destroy).pack(side="right")
        ttk.Button(bar, text="Apply", command=self._apply).pack(side="right", padx=6)
        self.bar = bar

    # ------------------------------------------------------------ updates
    def _check_updates(self):
        """Ask GitHub for the newest release. Nothing is downloaded or installed until you say so."""
        from . import updater
        from .version import UPDATE_REPO, __version__
        repo = getattr(self.engine.cfg, "update_repo", "") or UPDATE_REPO
        if not repo:
            self.update_lbl.config(text="Updates are not set up in this build.")
            return
        self.update_btn.config(state="disabled")
        self.update_lbl.config(text="checking...")

        def work():
            try:
                rel = updater.check(repo, __version__)
                newest = rel.version if rel else updater.newest_version(repo)
                self.app.q.put(("call", lambda: self._update_found(rel, newest)))
            except Exception as e:
                self.app.q.put(("call", lambda e=e: self._update_failed(updater.explain(e))))
        threading.Thread(target=work, daemon=True).start()

    def _update_failed(self, msg: str):
        if self.winfo_exists():
            self.update_btn.config(state="normal")
            self.update_lbl.config(text=msg[:260])

    def _update_found(self, rel, newest: str = ""):
        """One button, no dialog, no link: after a check it reads 'Update now to X' and a click does the rest."""
        from . import updater
        from .version import __version__
        if not self.winfo_exists():
            return
        plan = updater.plan(rel, __version__, updater.can_self_update(), newest)
        self.update_lbl.config(text=plan.label)
        if plan.action == "install":
            self.update_btn.config(state="normal", text=plan.button, command=lambda: self._update_now(rel))
        else:
            self.update_btn.config(state="normal", text=plan.button, command=self._check_updates)

    def _update_now(self, rel):
        """Download from GitHub, check the SHA-256, hand over to Setup; the app closes and reopens."""
        from . import updater
        self.update_btn.config(state="disabled")
        self.update_lbl.config(text="downloading...")

        def work():
            try:
                def progress(done, total):
                    if total:
                        self.app.q.put(("call", lambda d=done, t=total: self.update_lbl.config(
                            text=f"downloading {d / 1e6:.0f} / {t / 1e6:.0f} MB") if self.winfo_exists() else None))
                path = updater.download(rel, progress)
                self.app.q.put(("call", lambda: self._install(path)))
            except Exception as e:
                self.app.q.put(("call", lambda e=e: self._update_retry(rel, f"update failed: {e}")))
        threading.Thread(target=work, daemon=True).start()

    def _update_retry(self, rel, msg: str):
        if self.winfo_exists():
            self.update_lbl.config(text=msg[:160])
            self.update_btn.config(state="normal", text=f"Try again ({rel.version})", command=lambda: self._update_now(rel))

    INSTALL_NOTICE_S = 3

    def _install(self, path):
        from . import updater
        version = path.stem.rsplit("-", 1)[-1] if "-" in path.stem else ""
        self.engine.note(f"installing update {path.name}; the app will close and reopen")
        if self.winfo_exists():
            self.update_lbl.config(text="Installing. The app closes now; the installer can take a minute or two "
                                        "to appear (Windows checks it first), then the app reopens by itself.")
            try:
                self.update_idletasks()
                import time as _t
                _t.sleep(self.INSTALL_NOTICE_S)              # long enough to read before the window closes
            except Exception:
                pass
        try:
            updater.install_and_restart(path, version=version)
        except Exception as e:                            # Setup could not even be started: say so, stay open
            log.exception("could not start Setup")
            self._update_failed(f"Could not start the installer: {e}")
            return
        self.app._quit()

    # ------------------------------------------------------------ emulators
    def _emulators(self):
        return [(r, d) for r in self.engine.runners for d in r.devices if hasattr(d, "power_off")]

    def _build_emulators(self):
        f = self.emu_tab
        for w in f.winfo_children():
            w.destroy()
        cpus = os.cpu_count() or 4
        heads = ("Emulator", "CPU cores", "RAM (MB)", "Frame rate", "Graphics", "Pin to cores", "Priority")
        for c, h in enumerate(heads):
            ttk.Label(f, text=h, font=("Segoe UI", 9, "bold")).grid(row=0, column=c, sticky="w", padx=6, pady=(0, 4))
        self.emu_rows = []
        for n, (runner, dev) in enumerate(self._emulators(), start=1):
            try:
                hw = A.read_hw(dev.avd)
            except Exception as e:
                ttk.Label(f, text=dev.name).grid(row=n, column=0, sticky="w", padx=6)
                ttk.Label(f, text=f"cannot read its settings: {e}", style="Bad.TLabel").grid(row=n, column=1, columnspan=5, sticky="w")
                continue
            v = {"cores": tk.StringVar(value=str(hw.cores)), "ram": tk.StringVar(value=str(hw.ram_mb)),
                 "fps": tk.StringVar(value=f"{hw.fps} fps"),
                 "gpu": tk.StringVar(value=GPU_LABELS[hw.gpu]), "pin": tk.StringVar(value=dev.cpu_affinity or ""),
                 "prio": tk.StringVar(value=PRIORITY_LABELS.get(dev.priority or "normal", "normal"))}
            ttk.Label(f, text=f"{dev.name}   ({dev.avd})").grid(row=n, column=0, sticky="w", padx=6, pady=3)
            ttk.Spinbox(f, from_=1, to=min(16, cpus), width=5, textvariable=v["cores"]).grid(row=n, column=1, padx=6)
            ttk.Combobox(f, values=RAM_CHOICES, width=7, textvariable=v["ram"]).grid(row=n, column=2, padx=6)
            ttk.Combobox(f, values=[f"{x} fps" for x in A.FPS_CHOICES], width=7, state="readonly",
                         textvariable=v["fps"]).grid(row=n, column=3, padx=6)
            ttk.Combobox(f, values=list(GPU_LABELS.values()), width=26, state="readonly",
                         textvariable=v["gpu"]).grid(row=n, column=4, padx=6)
            ttk.Entry(f, width=12, textvariable=v["pin"]).grid(row=n, column=5, padx=6)
            ttk.Combobox(f, values=list(PRIORITY_LABELS.values()), width=13, state="readonly",
                         textvariable=v["prio"]).grid(row=n, column=6, padx=6)
            self.emu_rows.append((runner, dev, hw, v))
        last = len(self.emu_rows) + 2
        tools = ttk.Frame(f); tools.grid(row=last, column=0, columnspan=7, sticky="w", pady=(10, 4))
        ttk.Button(tools, text="Suggest separate cores", command=self._suggest).pack(side="left")
        ttk.Button(tools, text="Clear pinning", command=lambda: [v["pin"].set("") for *_x, v in self.emu_rows]).pack(side="left", padx=6)
        ttk.Button(tools, text="Add emulator account...", command=self._add_emulator).pack(side="left", padx=(24, 0))
        try:
            import ctypes

            class MS(ctypes.Structure):
                _fields_ = [("l", ctypes.c_ulong), ("load", ctypes.c_ulong), ("total", ctypes.c_ulonglong),
                            ("avail", ctypes.c_ulonglong)] + [(f"x{i}", ctypes.c_ulonglong) for i in range(5)]
            ms = MS(); ms.l = ctypes.sizeof(MS); ctypes.windll.kernel32.GlobalMemoryStatusEx(ctypes.byref(ms))
            ram = f"{ms.total / 2**30:.0f} GB RAM ({ms.avail / 2**30:.0f} GB free)"
        except Exception:
            ram = "RAM unknown"
        help_text = (
            f"This PC: logical processors 0-{cpus - 1}, {ram}.\n\n"
            "CPU cores and RAM are the most an emulator may use, not a reservation.\n"
            "To use less CPU and power: 2 cores and 30 fps. Measured on one emulator playing a live stream: 4 cores/60 fps "
            "2.2 CPUs, 2 cores/30 fps 1.4 CPUs (about a third less), and the bot's screen reads get about 0.2 s slower. A "
            "smaller screen size and Windows' efficiency mode were also measured and changed nothing.\n"
            "Graphics: leave it on 'default'. Measured: the emulator already draws on the graphics card by itself, and an "
            "emulator costs about 2 CPU cores while a live stream plays whatever this is set to. 'software' draws on the CPU "
            "instead and exists only for PCs where an emulator's picture misbehaves.\n"
            "Pin to cores is what really dedicates CPU: e.g. 28-31 keeps that emulator on those four logical processors and "
            "off the rest. Blank = not pinned. Priority 'below normal' makes emulators give way to games.\n\n"
            "Cores, RAM and graphics need that emulator restarted (a cold boot, about 20 s); you are asked after Apply. "
            "Pinning and priority apply immediately, and the bot re-applies them after every boot because Windows forgets them.")
        ttk.Label(f, text=help_text, wraplength=760, justify="left", style="Body.TLabel").grid(
            row=last + 1, column=0, columnspan=7, sticky="w", pady=(8, 0))

    def _suggest(self):
        want = []
        for *_x, v in self.emu_rows:
            try:
                want.append(max(1, int(v["cores"].get())))
            except ValueError:
                want.append(2)
        for (*_x, v), text in zip(self.emu_rows, suggest_pinning(len(want), want, os.cpu_count() or 4)):
            v["pin"].set(text)

    def _add_emulator(self):
        self.app._add_emulator()
        self.after(8000, self._build_emulators)       # the AVD takes a few seconds to create

    # ------------------------------------------------------------ bot
    BOT_FIELDS = (
        ("pacing", "max_entries_per_hour", "Entries per hour, per account", 1, 200,
         "Hard cap. Unconfirmed taps count too."),
        ("scoring", "min_viewers_to_camp", "Smallest stream the scanner will sit in (viewers)", 0, 5000, ""),
        ("pacing", "switch_if_never_active_s", "Leave a stream that has shown no giveaway after (seconds)", 30, 3600,
         "A stream with a known rhythm is given one and a half of its own intervals instead."),
        ("pacing", "switch_after_quiet_s", "Leave a stream that HAS run giveaways after this many quiet seconds", 30, 7200, ""),
        ("pacing", "quiet_cooldown_s", "After leaving a quiet stream, put it last for (seconds)", 0, 86400,
         "Stops the scanner going straight back."),
        ("pacing", "single_quiet_s", "Stay in one stream: back to Scan after this many seconds with no giveaway", 60,
         86400, "600 = 10 min. The stay also ends when the stream ends. 9 in 10 giveaways follow the previous one "
                "within 11 min."),
        ("whatnot", "feed_max_pages", "How deep to look into each category (pages of 50 streams)", 1, 12,
         "12 = up to 600 streams per category, smallest included. 1 = only the top 50."),
        ("whatnot", "discovery_interval_s", "Re-check which streams are live every (seconds)", 30, 3600,
         "60 is the default. This is also how the bot notices a stream ended, so longer = slower to move on."),
        ("hop", "max_per_hour", "Voluntary moves per account per hour, at most", 0, 3,
         "Crowd hops and pack-only upgrades together. 0 = never hop."),
    )

    PERIODS = (("Last hour", 3600), ("Last 24 hours", 86400), ("Last 7 days", 7 * 86400))

    def _build_results(self):
        """Per account: entered, unconfirmed, skipped, missed (including gaps in the giveaway numbering), the share
        of wanted giveaways caught, wins, and the median number of entrants. See givvy/metrics.py."""
        f = self.results_tab
        top = ttk.Frame(f); top.pack(fill="x")
        self.results_period = tk.StringVar(value=self.PERIODS[1][0])
        ttk.Combobox(top, textvariable=self.results_period, values=[p[0] for p in self.PERIODS], state="readonly",
                     width=14).pack(side="left")
        ttk.Button(top, text="Refresh", command=self._fill_results).pack(side="left", padx=8)
        cols = ("entered", "unconfirmed", "skipped", "missed", "caught", "wins", "entrants")
        heads = ("Entered", "Unconfirmed", "Skipped", "Missed", "Caught", "Wins", "Median entrants")
        self.results_tree = ttk.Treeview(f, columns=cols, show="tree headings", height=6)
        self.results_tree.heading("#0", text="Account"); self.results_tree.column("#0", width=120)
        for c, h in zip(cols, heads):
            self.results_tree.heading(c, text=h); self.results_tree.column(c, width=90, anchor="e")
        self.results_tree.pack(fill="both", expand=True, pady=8)
        ttk.Label(f, style="Muted.TLabel", justify="left", text=(
            "Missed counts the giveaways the bot knew it missed, plus gaps in the sellers' numbering (#11 then #13 in "
            "one stream = #12 went by).\nCaught = entered or unconfirmed, out of everything wanted. Unconfirmed = Enter "
            "was tapped but never proven either way.\nWins come from the win history (winner dialog and Givvy Wins "
            "tracker). Rows of one giveaway recorded twice count once.")).pack(anchor="w")
        self._build_trial(f)
        self.results_period.trace_add("write", lambda *a: self._fill_results())
        self._fill_results()

    def _build_trial(self, f):
        """The 1.2.0 trial report (givvy/trial.py), the same text as --trial-report, and the button that moves the
        trial's start after a change in the middle of it."""
        box = ttk.LabelFrame(f, text="Trial: this version against before")
        box.pack(fill="both", expand=True, pady=(10, 0))
        bar = ttk.Frame(box); bar.pack(fill="x", padx=6, pady=(6, 0))
        ttk.Button(bar, text="Start a new trial window now", command=self._new_trial_window).pack(side="left")
        start = self.engine.cfg.trial.start
        ttk.Label(bar, style="Muted.TLabel", text=(
            f"[trial] start = {start!r} in config.toml wins over this button until you remove it." if start else
            "The same text as --trial-report, which writes it to trial_report.txt.")).pack(side="left", padx=8)
        body = ttk.Frame(box); body.pack(fill="both", expand=True, padx=6, pady=6)
        ys, xs = ttk.Scrollbar(body, orient="vertical"), ttk.Scrollbar(body, orient="horizontal")
        # 10 lines, scrolled: at 26 the Results tab alone asked for 717 px and the window for 798 (574 at 10).
        self.trial_text = tk.Text(body, height=10, width=110, wrap="none", font=("Consolas", 9),
                                  yscrollcommand=ys.set, xscrollcommand=xs.set)
        ys.config(command=self.trial_text.yview); xs.config(command=self.trial_text.xview)
        ys.pack(side="right", fill="y"); xs.pack(side="bottom", fill="x")
        self.trial_text.pack(side="left", fill="both", expand=True)

    def _new_trial_window(self):
        """The trial is counted from now, after a change in the middle of it (an account added or removed, the crowd hop
        switched off). The baseline stays before this version's first start (givvy/trial.py windows)."""
        from . import trial
        from .version import short_version
        eng = self.engine
        text = ("Start a new trial window now?\n\nThe trial will be counted from this moment. The baseline stays the days "
                "before this version first started, so it never takes in this version's own days. Do this after a "
                "change in the middle of the trial: an account added or removed, the crowd hop switched off.")
        if eng.cfg.trial.start:
            text += f"\n\n[trial] start = {eng.cfg.trial.start!r} in config.toml still wins until you remove it."
        if not messagebox.askyesno("Trial", text, parent=self):
            return
        now, label = time.time(), short_version()
        trial.mark(eng.store, now, label, eng.trial_settings())
        eng.note(f"trial window starts {time.strftime('%Y-%m-%d %H:%M', time.localtime(now))} ({label})")
        self._fill_results()

    def _fill_results(self):
        from . import metrics, trial
        seconds = dict(self.PERIODS).get(self.results_period.get(), 86400)
        try:
            stats = metrics.report(self.engine.store, since=time.time() - seconds)
        except Exception as e:
            log.exception("results")
            stats = {}
        self.results_tree.delete(*self.results_tree.get_children())
        for account in sorted(stats):
            s = stats[account]
            self.results_tree.insert("", "end", text=account, values=(
                s.entered, s.unconfirmed, s.skipped, s.missed, f"{s.caught:.0%}" if s.caught is not None else "-",
                s.wins, s.median_entrants if s.median_entrants is not None else "-"))
        try:
            text = trial.trial_report(self.engine.store, self.engine.cfg,
                                      accounts=self.engine.trial_settings()["accounts"])
        except Exception as e:
            log.exception("trial report")
            text = f"The trial report failed: {e}"
        self.trial_text.config(state="normal")
        self.trial_text.delete("1.0", "end")
        self.trial_text.insert("1.0", text)
        self.trial_text.config(state="disabled")

    def _build_blacklist(self):
        import time
        f = self.black_tab
        for w in f.winfo_children():
            w.destroy()
        ttk.Label(f, text="Sellers the scanner never picks. Added by the 'Blacklist stream' button, by the tariff notice,\n"
                          "or by a giveaway that is only for another country.", justify="left").pack(anchor="w")
        self.black_list = tk.Listbox(f, height=12, width=78, activestyle="none")
        self.black_list.pack(fill="both", expand=True, pady=8)
        self._black_rows = self.engine.store.blacklist_rows()
        for seller, reason, added in self._black_rows:
            when = time.strftime("%Y-%m-%d %H:%M", time.localtime(added)) if added else ""
            self.black_list.insert("end", f"{seller:<32} {reason or '':<28} {when}")
        self.black_list.config(font=("Consolas", 9))
        ttk.Button(f, text="Remove from blacklist", command=self._unblacklist).pack(anchor="w")

    def _unblacklist(self):
        sel = self.black_list.curselection()
        if not sel:
            return
        seller = self._black_rows[sel[0]][0]
        self.engine.store.blacklist_remove(seller)
        self.engine.note(f"{seller} removed from the blacklist")
        self._build_blacklist()
        self.app._fill_tree(self.engine.shows)

    def _build_bot(self):
        f = self.bot_tab
        self.bot_vars = {}
        for n, (section, key, label, lo, hi, note) in enumerate(self.BOT_FIELDS):
            cur = getattr(getattr(self.engine.cfg, section), key)
            var = tk.StringVar(value=str(int(cur)))
            ttk.Label(f, text=label).grid(row=n, column=0, sticky="w", pady=4)
            ttk.Spinbox(f, from_=lo, to=hi, width=8, textvariable=var).grid(row=n, column=1, padx=10)
            ttk.Label(f, text=note, style="Muted.TLabel").grid(row=n, column=2, sticky="w")
            self.bot_vars[(section, key)] = (var, lo, hi, label)
        ttk.Label(f, text="Saved to config.toml and used straight away; no restart needed.\n"
                          "Everything else (scoring weights, Discord, pacing) is in config.toml next to the program.",
                  style="Body.TLabel", justify="left").grid(row=len(self.BOT_FIELDS), column=0, columnspan=3, sticky="w", pady=(12, 0))
        # Real alerts are rare (an emulator that will not start, a safety pause, a sign-in, an account out of a stream
        # for 15 min), so check once, on this PC, that they reach you.
        ttk.Button(f, text="Send a test alert", command=self._test_alert).grid(
            row=len(self.BOT_FIELDS) + 1, column=0, sticky="w", pady=(12, 0))
        ttk.Label(f, text="A Windows notification, the red banner in the main window, a line in alerts.log, and Discord "
                          "when it is set up. It clears itself after 15 seconds.",
                  style="Muted.TLabel", wraplength=520, justify="left").grid(
            row=len(self.BOT_FIELDS) + 1, column=1, columnspan=2, sticky="w", pady=(12, 0))
        # The crowd hop's one switch ([hop] mode): the 1.2.0 trial may need it off, or back to log only, at once.
        row = len(self.BOT_FIELDS) + 2
        ttk.Label(f, text="Leave a crowded stream right after its draw").grid(row=row, column=0, sticky="w", pady=(12, 0))
        self.hop_mode = tk.StringVar(value=HOP_LABELS.get(self.engine.cfg.hop.mode, HOP_LABELS["off"]))
        ttk.Combobox(f, textvariable=self.hop_mode, values=list(HOP_LABELS.values()), state="readonly",
                     width=22).grid(row=row, column=1, padx=10, pady=(12, 0), sticky="w")
        ttk.Label(f, text=f"Only for a free stream whose giveaway just started and that promises "
                          f"{self.engine.cfg.hop.margin:g}x the pack wins per hour, at most the moves per hour above. "
                          f"'log only' records what it would do and never moves; 'off' is how 1.1 behaved.",
                  style="Muted.TLabel", wraplength=520, justify="left").grid(row=row, column=2, sticky="w", pady=(12, 0))

    def _test_alert(self):
        alerts = self.engine.alerts
        alerts.fire("test", "Test alert from Whatnot Givvy: if you can read this, alerts reach you here.")
        # On the main window, not this one: closed within 15 s, this window would take the silent resolve with it.
        self.app.after(15000, lambda: alerts.resolve("test"))

    # ------------------------------------------------------------ apply
    def _apply(self):
        cfg_path = self.engine.cfg.root / "config.toml"
        try:
            bot_changes = []
            for (section, key), (var, lo, hi, label) in self.bot_vars.items():
                try:
                    val = int(var.get())
                except ValueError:
                    raise ValueError(f"{label}: a whole number, please")
                if not lo <= val <= hi:
                    raise ValueError(f"{label}: {lo} to {hi}")
                if val != int(getattr(getattr(self.engine.cfg, section), key)):
                    bot_changes.append((section, key, val))
            emu_changes = []
            for runner, dev, old_hw, v in self.emu_rows:
                try:
                    hw = A.Hardware(int(v["cores"].get()), int(v["ram"].get()), _key(GPU_LABELS, v["gpu"].get()),
                                    int(v["fps"].get().split()[0]))
                except ValueError:
                    raise ValueError(f"{dev.name}: cores and RAM must be whole numbers")
                pin, prio = v["pin"].get().strip(), _key(PRIORITY_LABELS, v["prio"].get())
                A.affinity_mask(pin)                              # validates; raises with a readable message
                if hw.cores < 1 or hw.ram_mb < 1024:
                    raise ValueError(f"{dev.name}: at least 1 core and 1024 MB")
                emu_changes.append((runner, dev, old_hw, hw, pin, prio, v))
        except ValueError as e:
            messagebox.showerror("Settings", str(e), parent=self)
            return

        for section, key, val in bot_changes:
            A.set_config_value(cfg_path, section, key, val)
            setattr(getattr(self.engine.cfg, section), key, val)
            self.engine.note(f"setting {key} = {val}")
        hop_mode = _key(HOP_LABELS, self.hop_mode.get())
        if hop_mode != self.engine.cfg.hop.mode:
            A.set_config_value(cfg_path, "hop", "mode", hop_mode)       # appends [hop] to an older config.toml
            self.engine.cfg.hop.mode = hop_mode                         # every account reads it at its next draw
            self.engine.note(f"setting crowd hop = {hop_mode}")

        restart = []
        for runner, dev, old_hw, hw, pin, prio, v in emu_changes:
            try:
                if hw != old_hw:
                    A.write_hw(dev.avd, hw)
                    self.engine.note(f"{dev.name}: {hw.cores} cores, {hw.ram_mb} MB, {hw.fps} fps, graphics {hw.gpu}")
                    if dev.is_running():
                        restart.append((runner, dev))
                if pin != (dev.cpu_affinity or "") or prio != (dev.priority or "normal"):
                    A.set_account_keys(cfg_path, dev.name, {"cpu_affinity": pin, "priority": prio})
                    dev.cpu_affinity, dev.priority = pin, prio
                    for a in self.engine.cfg.accounts:
                        if a.name == dev.name:
                            a.cpu_affinity, a.priority = pin, prio
                    self.engine.note(f"{dev.name}: pinned to [{pin or 'all cores'}], priority {PRIORITY_LABELS[prio]}")
                    threading.Thread(target=self._tune_now, args=(dev, pin, prio), daemon=True).start()
            except Exception as e:
                log.exception("applying settings for %s", dev.name)
                messagebox.showerror("Settings", f"{dev.name}: {e}", parent=self)
                return
        self._build_emulators()
        if restart:
            names = ", ".join(d.name for _r, d in restart)
            if messagebox.askyesno("Settings", f"Restart {names} now so the new cores / RAM / graphics take effect?\n\n"
                                               "It is a clean power-off and a cold boot, about 20 seconds each. "
                                               "Your Whatnot sign-in is kept.", parent=self):
                for runner, dev in restart:
                    runner.power(dev.name, "restart")
            else:
                self.engine.note(f"{names}: new hardware settings take effect at the next Restart")

    @staticmethod
    def _tune_now(dev, pin: str, prio: str):
        """Applies to a running emulator immediately. Un-pinning means 'all cores'."""
        try:
            mask = A.affinity_mask(pin) or (1 << min(os.cpu_count() or 1, 64)) - 1
            A.apply_process_tuning(dev.avd, mask, prio)
        except Exception:
            log.exception("could not apply pinning to %s", dev.name)
