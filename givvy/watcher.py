...
class Watcher:
    ...
    def plan(self, shows: list[LiveShow], followed: set[str]) -> tuple[list[str], list[str]]:
        ...
        if order:
            # The engine's choice: the streams the scanner would sit in, best first. It used to be the
            # BIGGEST streams, which told the scanner nothing about the small pack streams it chooses among.
            by_id = {s.id: s for s in shows}
            desired = [by_id[i] for i in order if i in by_id][: self.slots]
        else:
    ...
    def set_shows(self, shows: list[LiveShow], followed: set[str], order: list[str] | None = None):
    ...
    # The two below are called from the camper threads (the crowd hop) while this asyncio thread changes `states`, or
    # replaces it whole on a fresh socket. A dict that changes size while it is copied raises RuntimeError: that read
    # counts as nothing running.
    def giveaway_running(self, show_id: str) -> tuple[str, float, bool] | None:
        """(live_product_id, started_at, saw_start) of the giveaway running in that stream, or None. A stream the crowd
        hop moves to was chosen for such a giveaway, whose 'active' came before the account was camped there."""
        try:
            st = self.states.get(show_id)
        except RuntimeError:
            return None
        ev = st.active_event if st is not None else None
        return (ev.live_product_id, ev.started_at, ev.saw_start) if ev is not None else None

    def fresh_giveaways(self) -> dict[str, float]:
        """show_id -> started_at of every running giveaway the socket SAW start. After one of the 5-8 socket reconnects
        an hour started_at is only the tune-in time (saw_start False), so those are left out."""
        try:
        ...
        for st in states:
            ev = st.active_event
            if ev is not None and ev.saw_start:
                out[st.show.id] = ev.started_at
        return out
