...
def list_devices(sdk: str) -> dict[str, str]:
    """serial -> state. Raises when adb itself failed: when it cannot reach its daemon (killed by another adb version
    or by `adb kill-server`, crashed, or failing to restart) it exits 1 with nothing on stdout, which used to parse as
    'no devices'. The emulator code took that for 'adb answered and does not list ours' and ended a healthy emulator
    as a leftover. Every caller treats an exception as 'cannot tell'."""
    r = subprocess.run([adb_path(sdk), "devices", "-l"], capture_output=True, text=True, timeout=30,
                       creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0))
    if r.returncode != 0 or "List of devices attached" not in (r.stdout or ""):
        raise RuntimeError(f"adb devices failed (exit {r.returncode}): {(r.stderr or r.stdout or '').strip()[:200]}")
