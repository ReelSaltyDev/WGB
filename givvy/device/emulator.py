...
class EmulatorDevice(AndroidDevice):
    ...
    def is_available(self) -> bool:
        ...
        try:
            ...
            if up and not self._data_checked and time.time() >= getattr(self, "_data_retry_at", 0.0):
                self._data_retry_at = time.time() + 30       # Wi-Fi not up yet, or still booting: not every tick
                self.quiet_data_warnings()
            return up
    ...
    def quiet_data_warnings(self) -> bool:
        """Stop Android's 'data warning' notifications. An emulator has a pretend mobile connection beside
        its Wi-Fi and counts the traffic against that 'plan' (2 GB warning; live video passes it within
        the hour). Where the Wi-Fi has working internet, switching the pretend mobile data off changes
        nothing else: measured, the stream downloads exactly as before. It is checked afterwards and put
        back if the emulator is left without a connection. True = nothing (more) to do."""
        if self._data_checked or getattr(self.dcfg, "emulator_mobile_data", False):
            return True
        try:
            if not self._wifi_has_internet(self._shell("dumpsys", "connectivity")):
                if not getattr(self, "_wifi_toggled", False):
                    # Measured: Wi-Fi off and on again gave it its route back and Android validated it
                    # within 6 s. Once per start; the mobile connection carries the stream meanwhile.
                    self._wifi_toggled = True
                    self._shell("svc", "wifi", "disable")
                    time.sleep(min(self.settle_s, 4.0))
                    self._shell("svc", "wifi", "enable")
                    log.info("%s: Wi-Fi had no working internet; switched it off and on", self.name)
                return False                         # keep the mobile connection; look again later
            if self._shell("settings", "get", "global", "mobile_data").strip() != "0":
                ...
                if not self._wifi_has_internet(self._shell("dumpsys", "connectivity")):
                    ...
                    self._data_checked = True        # do not flap: leave this one as it was
                    return True
                log.info("%s: simulated mobile data switched off (no more Android data warnings); Wi-Fi carries on", self.name)
            self._data_checked = True
            return True
        except Exception as e:
            log.warning("%s: could not check mobile data: %s", self.name, e)
            return False
            ...
            if self.is_available():
                try:
                    if self._shell("getprop", "sys.boot_completed").strip() == "1":
                        ...
                        self._tune()
                        self.quiet_data_warnings()   # Wi-Fi may not be up yet: is_available() asks again
                        return True
                except Exception:
                    pass
