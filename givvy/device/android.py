...
class AndroidDevice:
    ...
    def _nodes(self) -> list[dict]:
        ...
        raw = self.adb.exec_out(self.dcfg.android_sdk, self.serial, "cat", DUMP_PATH)
        return S.parse_nodes(raw.decode("utf-8", errors="replace"))
    ...
    def expand_then_read(self, ns: list[dict] | None = None
        ...
        if self.dry_run:
            ...
            log.info("tap badge at %s (coords from a read %.1fs old)", xy, time.time() - self._read_at)
            self._shell("input", "tap", str(xy[0]), str(xy[1]))
