...
# Seen live (screenshot 2026-09-19): where the Enter button would be, a pill reading 'AU Only Giveaway'.
# Whole line, a country code or name only: 'Followers Only Giveaway' must not match.
_COUNTRIES = ("australia|canada|united kingdom|united states|new zealand|germany|france|austria|belgium|"
              "netherlands|ireland|spain|italy|japan|mexico|sweden|switzerland|poland|portugal")
REGION_ONLY = re.compile(r"^([A-Za-z]{2,3}|" + _COUNTRIES + r")\s+only\s+giveaway$", re.I)


def region_only(nodes: list[dict]) -> str:
    """'AU' when the giveaway card says 'AU Only Giveaway', else ''. Card area only: chat can say anything."""
    h = _screen_height(nodes)
    for n in nodes:
        m = REGION_ONLY.match(text_of(n).strip())
        if m and _in_card_zone(n, h):
...
JOINED = re.compile(r"^joined\b", re.I)
USERNAME = re.compile(r"^@?([A-Za-z0-9_.]{2,30})$")


def joined_names(nodes: list[dict]) -> set[str]:
    """Usernames in the chat's '<username> joined ..' lines. Read live (emulator 3, 9/23): three nodes in order, the
    avatar letter, the username, then 'joined ..'; so the name is the text just before each 'joined'."""
    out = set()
    for i, n in enumerate(nodes):
        if not JOINED.match(text_of(n)):
            continue
        for prev in reversed(nodes[max(0, i - 2):i]):
            m = USERNAME.match(text_of(prev))
...
def badge_icon_box(nodes: list[dict]) -> tuple[int, int, int, int] | None:
    """Bounds of the small icon on the COLLAPSED giveaway badge (a gift, or a tick once we are in). It is the
    node described as 'Giveaway' with no text of its own; the label beside it has the text. See device/tick.py."""
    if badge_tap_point(nodes) is None:
        return None
    for n in nodes:
        if (n.get("content-desc") or "").strip().lower() == "giveaway" and not (n.get("text") or "").strip():
            return bounds_of(n)
    return None


ENTER_BELOW_BADGE_PX = 144     # on a 1080-wide screen. Measured 2026-09-20: badge tapped at y 327|353, Enter at
                               # y 471|497, x 540, in 463 taps out of 463.


def enter_point_from_badge(nodes: list[dict]) -> tuple[int, int] | None:
    """Where 'Enter Giveaway' will be once the collapsed badge on this screen has been tapped open:
    centred, a fixed distance below the badge's tap point. None when there is no badge."""
    tap = badge_tap_point(nodes)
    if tap is None:
        return None
    width = max((b[2] for b in (bounds_of(n) for n in nodes) if b), default=1080) or 1080
    return (width // 2, tap[1] + round(ENTER_BELOW_BADGE_PX * width / 1080))
