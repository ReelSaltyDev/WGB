...
def describe(listings, seen: str = "", limit: int | None = SHOWN) -> str:
    ...
    for g in shown:
        ...
        if g.quantity and g.quantity > 1:
            t += f" ×{g.quantity}"
        if g.buyer_appreciation:
            t += " [buyers only]"                    # cannot be entered without a purchase; worth knowing
        items.append(t)
    text = "  ·  ".join(items)
...
class GiveawayDirectory:
    ...
    def enterable_counts(self) -> dict[str, int]:
        ...
        with self._lock:
            return {s: sum(1 for g in ls if not g.buyer_appreciation) for s, ls in self._listings.items()}
