...
class Listed:
    ...
    pack_share: float = field(default=-1.0, compare=False)   # packs / all listed, by quantity left; -1 = not worked out
...
class GiveawayDirectory:
    ...
    def refresh(self, show_ids_by_interest: list[str], now: float | None = None, forget: bool = True) -> int:
        """Fetch up to `per_round` streams that are missing or stale. Call it every
        few seconds from a background thread; it is the rate limit. Streams left out of
        the list are dropped, unless `forget` is False (the list is not a real stream list)."""
        now = time.time() if now is None else now
        live = set(show_ids_by_interest)
        with self._lock:
            for dead in [s for s in self._tried_at if s not in live] if forget else []:
    ...
    def summary(self) -> dict[str, Listed]:
        ...
        pack_share: the quantity left of each listing is the closest thing the web API gives to the mix that will
        actually run. Buyers giveaways count in the total, because the start rate counts them too. Only the
        pack-only upgrade (and the crowd hop) use it, never rank_streams: the kind-first order is untouched."""
        q = lambda g: max(1, int(g.quantity or 1))
        with self._lock:
            out = {}
            for s, ls in self._listings.items():
                free = [g for g in ls if not is_buyers_only(g)]
                grades = [grade_listing(g) for g in free]
                grade = max(grades, default=-1)
                packs = sum(q(g) for g, gr in zip(free, grades) if gr == 2)
                every = sum(q(g) for g in ls)
                ...
                out[s] = Listed(open=len(free), pack=grade == 2, buyers=len(ls) - len(free), grade=grade,
                                ...
                                pack_share=packs / every if every else 0.0)
            return out
