...
class Discovery:
    ...
    def poll(self, on_feeds=None) -> list[LiveShow]:
        """Every live stream worth knowing about. `on_feeds(shows)`, when given, gets the category-feed results
        (filtered and ordered like the full list) before the followed sellers are looked up. Those lookups are one
        request each, one after another: 247 followed sellers and 6 favorites on 9/24, and on 9/23-24 they held the
        first pick after every start for 79-89 s, while 494 of the 498 moves since 9/20 were to pokemon-category
        streams, which the feeds carry. A failing `on_feeds` does not stop the lookups."""
        ...
        if on_feeds is not None:
            try:
                on_feeds(self._finish(found))
            except Exception:
                log.exception("could not hand over the category feeds")
    ...
    def _finish(self, found: dict[str, LiveShow]) -> list[LiveShow]:
        """The home-country filter and the order, as a new list: `found` is left as it is."""
