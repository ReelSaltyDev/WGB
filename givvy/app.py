...
REFRESH_MS = 700
TODAY_INK, TODAY_FLASH, FLASH_MS = "#202124", "#1e8e3e", 5000
...
class App(tk.Tk):
    ...
    def _apply_charts_visibility(self):
        ...
        if self._ui.get("charts_shown", False):
            after = self.right if self.right.winfo_manager() else self.favs_frame
            self.charts.pack(side="left", fill="both", expand=True, padx=(10, 0), after=after)
    ...
    def _build(self):
        ...
        ttk.Label(foot, text=f"Whatnot Givvy {short_version()}", foreground="#777",
        ...
        self.today_lbl = ttk.Label(top, text="Total Wins Today: 0", font=("Segoe UI", 22, "bold"),
                                   foreground=TODAY_INK)
        self.today_lbl.pack(side="left", expand=True)
    ...
    def _show_wins_today(self):
        ...
        if went_up:
            ...
            self.today_lbl.config(foreground=TODAY_FLASH)
            self._today_flash = self.after(FLASH_MS, self._end_flash)

    def _end_flash(self):
        self._today_flash = None
        self.today_lbl.config(foreground=TODAY_INK)
