...
class Engine:
    ...
    def _make_runner(self, label: str, accs) -> AccountRunner:
        ...
        camper.listed_lookup = self.giveaways.enterable_counts
    ...
    def add_account(self, a) -> AccountRunner:
        ...
        self._retag()
        if self.running:
            runner.camper.set_mode(self.runners[0].camper.state.mode)
    ...
    def _discover(self) -> bool:
        ...
        try:
            self.watcher.set_shows(shows, self.followed_any())
        except Exception:
            log.exception("watcher.set_shows")
    ...
    def rank_extras(self) -> dict:
        now = time.time()
        return {"listed": self.giveaways.enterable_counts(),
    ...
    def _camp_loop(self, r: AccountRunner):
        ...
        while not self._stop.is_set():
            ...
            self._stop.wait(max(0.2, r.camper.wanted_poll_interval() - (time.time() - t0)))
        self.claims.release(r.account)
...
class App(tk.Tk):
    ...
    def _apply_minsize(self):
        self.minsize(640 if self._ui.get("log_hidden") else 900, 480)
    ...
    def _build(self):
        ...
        ttk.Label(top, text="Mode (all accounts):", font=("Segoe UI", 10, "bold")).pack(side="left")
        self.mode_var = tk.StringVar(value="scan")
        for text, val in (("Scan streams", "scan"), ("Stay in one stream", "single")):
            ttk.Radiobutton(top, text=text, value=val, variable=self.mode_var,
                            command=self._mode_changed).pack(side="left", padx=6)

        ttk.Separator(top, orient="vertical").pack(side="left", fill="y", padx=14)
        ...
        for c, w, label in (("game", 90, "Game"), ("viewers", 80, "Viewers"), ("givvies", 80, "Givvies"),
                            ("score", 80, "Score"), ("held", 110, "Held by")):
        ...
        self.tree.column("prizes", width=520, anchor="w", stretch=True)
        self.prize_lbl = ttk.Label(left, text="", wraplength=900, justify="left", foreground="#333")
        ...
        self.updated_lbl = ttk.Label(pinbar, text="", foreground="#555")
    ...
    def _build_device_rows(self):
        ...
        for w in devs.winfo_children():
            w.destroy()
        heads = ("Use", "Device", "Account", "Emulator power / rename", "State", "Stream", "Entries/hr", "Last action")
        for c, h in enumerate(heads):
            ttk.Label(devs, text=h, font=("Segoe UI", 9, "bold")).grid(row=0, column=c, sticky="w", padx=8, pady=(4, 2))
        devs.columnconfigure(7, weight=1)
        self.rows = []
        self.power_btns = {}
        n = 1
        for r in self.engine.runners:
            ...
            for d in r.devices:
                ...
                box = ttk.Frame(devs); box.grid(row=n, column=3, sticky="w", padx=8)
                ...
                for c, lab in enumerate(labels, start=4):
    ...
    def _rename(self, device_name: str):
        ...
        ttk.Label(frm, text=hint, wraplength=380, foreground="#555").grid(row=2, column=0, columnspan=2, sticky="w", pady=6)
    ...
    def _mode_changed(self):
        for r in self.engine.runners:
            r.camper.set_mode(self.mode_var.get())
    ...
    def _tick(self):
        ...
        for runner, dev, var, (state_l, stream_l, cap_l, action_l) in self.rows:
            ...
            if busy:
                ...
                state = ("NEEDS SIGN-IN" if st.paused_reason == "login" else "PAUSED") if st.paused else "ACTIVE"
    ...
    def _fill_tree(self, shows):
        ...
        ranked = rank_streams(shows, eng.giveaway_counts, self.cfg, eng.followed_any(),
                              eng.store.blacklisted_sellers(), favorites=favs, **eng.rank_extras())
        held = eng.claims.snapshot()
        sel = self.tree.selection()
        ...
        for _lo, label in BANDS:
            ...
            if not picks:
                continue
            best = max(p.score for p in picks)
            self.tree.insert("", "end", iid="band:" + label, text=f"{label}   ({len(picks)} streams)",
                             values=("", "", "", f"best {best:.0f}", "", ""), open=bool(open_bands.get(label, False)))
            for p in picks:
                s = p.show
                self.tree.insert("band:" + label, "end", iid=s.id,
                                 ...
                                 values=(s.game, s.viewers, eng.giveaway_counts.get(s.id, 0),
                                         f"{p.score:.0f}", held.get(s.id, ""), eng.giveaways.text(s.id)))
