...
def paused_by_you(runners) -> bool:
    """Every account is paused with the window's Pause button. The bot's own pauses (a safety pause, a sign-in pause)
    do not count: counted, the button, which still read 'Pause', resumed every account when all had tripped the breaker
    together (a Whatnot-wide failure, or the friend's one account), clearing the safety pause and firing a stream link
    at the next step, which is the hammering the breaker exists to stop."""
    return bool(runners) and all(r.camper.state.paused and r.camper.state.paused_reason == "" for r in runners)
...
class Engine:
    ...
    def __init__(self, cfg, on_event):
        ...
        try:                                              # a stay cut off by a crash or kill ends at its last seen_at
            self.store.close_open_visits()
        except Exception:
            log.exception("could not close the stays a crash left open")
        self.notifier = Notifier(cfg.discord, skip_history=True)
        # Alerts reach you without Discord too: alerts.log, the window's banner and a Windows notification.
        self.alerts = Alerts(cfg.root / cfg.paths.alerts_log, cfg.discord, windows=cfg.alerts.windows_notify)
        ...
        self.activity_book = ActivityBook(sink=self.store.record_activity, quiet_max_per_h=cfg.scoring.quiet_max_per_h,
                                          quiet_memory_s=cfg.scoring.quiet_memory_s,
                                          crowd_from_seen=cfg.scoring.crowd_from_seen)
        try:
    ...
    def _make_runner(self, label: str, accs) -> AccountRunner:
        ...
        d = self.cfg.device
        runner.retry_s, runner.retry_max_s, runner.alert_after = (d.emulator_retry_s, d.emulator_retry_max_s,
                                                                  d.emulator_alert_after)
        runner.alerts = self.alerts
        ...
        if self.cfg.pacing.resume_after_restart:
            camper.load_saved_stream()            # the stream this account was in when the app last stopped
        camper.load_breaker()                     # a safety pause still running goes on (and skips that stream)
    ...
    def rename(self, old_name: str, new_name: str, new_account: str):
        ...
        for a in self.cfg.accounts:
            ...
            if a.name == old_name:
                a.name = new_name
        # The names a win is matched by ('<username> won the giveaway!'): they were only built at start, so an account
        # renamed to its Whatnot name went on missing its wins until the app was restarted.
        runner.camper.names.account = new_acc
        learned = runner.camper.names.name()
        by_name = {a.name: a for a in self.cfg.accounts}
        for d in runner.devices:
            a = by_name.get(d.name)
            if a is not None and hasattr(d, "usernames"):
                d.usernames = {u.strip().lower() for u in (a.whatnot_username, a.account, a.name, learned)
                               if u and u.strip()}
        self._retag()
    ...
    def _giveaway_seen(self, ev):
        ...
        elif ev.show.id not in self.watcher.states:
            return      # Watcher._reconcile drops a stream it stops listening to BEFORE sending its 'ended': not a draw
        # The account sitting in that stream gets both kinds for the crowd hop (Camper._hop_drain): a deque append on
        # this thread and popleft on its own is safe, the same hand-off as the wake. A start makes it look NOW, not at
        # its next scheduled poll (up to 3 s away). A draw wakes it only while the crowd hop is on: off stays exactly
        # as before, with no extra screen read.
        wake = ev.status == "active" or self.cfg.hop.mode in hop.ON
        now = time.time()
        for r in self.runners:
            camped = r.camper.state.camped
            if camped is not None and camped.id == ev.show.id:
                r.camper.socket_events.append((ev.status, ev.show.id, ev.live_product_id, now, ev.started_at,
                                               getattr(ev, "saw_start", True)))
                if wake:
                    r.wake.set()
    ...
    def _discover(self) -> bool:
        ...
        With no list yet (a start) and whatnot.feeds_first, the category feeds are handed to the accounts first and
        the full list follows the ~250 followed-seller lookups. 9/23-24 every start waited 79-89 s on those lookups
        ('off -> choosing: no suitable stream live') before either account could pick a stream."""
        cold = not self.shows and self.cfg.whatnot.feeds_first
        t0 = time.time()

        def feeds_found(shows):
            log.info("stream list: %d from the category feeds; looking up followed sellers", len(shows))
            self._publish(shows, partial=True)
        try:
            shows = self.discovery.poll(on_feeds=feeds_found) if cold else self.discovery.poll()
        except Exception as e:
            self.note(f"discovery failed: {e}")
            return False
        if cold:
            log.info("stream list: %d live, %d found only through followed sellers, after %.0f s", len(shows),
                     sum(1 for s in shows if s.source.startswith("seller:")), time.time() - t0)
        self._publish(shows)
        return True

    def _publish(self, shows, partial: bool = False) -> None:
        """A new stream list for the window, the accounts and the listeners. `partial`: the category feeds only (see
        _discover). It is never grounds for an account to leave a stream (Camper.set_shows), and it must not make the
        ActivityBook forget what the database gave back about streams found only through followed sellers."""
        self.shows = shows
        for r in self.runners:
            r.camper.set_shows(shows, complete=not partial)
        self.retarget_listeners(forget=not partial)
    ...
    def retarget_listeners(self, forget: bool = True) -> None:
        """`forget`: the ActivityBook drops every stream not in the list. False for a list that is known to be short
        of streams (the category feeds alone, _publish).

        The streams our accounts are in are kept either way, listed or not. An account stays in one a list pages past
        when Whatnot says it is still live (Camper._gone, about 1.6 false misses a day), and forgetting it wiped its
        rhythm, entrant reads and quiet record, while the socket topic was left (plan() only keeps streams it is given):
        no draws for the crowd hop, no starts to rebuild the rhythm. The crowd hop then valued it at the 2.0/h guess and
        left a stream worth twice the one it moved to (0.059 against 0.162, where it was 0.355)."""
        try:
            order = self.listen_order()
            camped = [c for c in (r.camper.state.camped for r in self.runners) if c is not None]
            ids = {s.id for s in self.shows}
            kept = list({c.id: c for c in camped if c.id not in ids}.values())
            self.watcher.set_shows(self.shows + kept, self.followed_any(), order)
            now = time.time()
            self.activity_book.watching(order[: self.cfg.whatnot.watch_slots], now)
            if forget:
                self.activity_book.forget(ids | {c.id for c in camped})
        except Exception:
    ...
    def watch_giveaway_lists(self):
        ...
        def loop():
            while True:
                try:
                    self._refresh_lists_once()
                except Exception:
                    log.exception("giveaway list refresh failed")
                time.sleep(4)
        threading.Thread(target=loop, daemon=True).start()

    def _refresh_lists_once(self) -> None:
        """One round of watch_giveaway_lists. It runs before the stream list is known too: the streams our accounts
        are in come first (interesting_show_ids), so a stream rejoined straight after a start has its giveaway list
        within about 4 s. Without any list the camper skips every card whose title does not name a pack
        (Camper._judge, 'legacy') and never fast-enters. The listeners are re-aimed only from a real stream list: with
        none, the ActivityBook would forget every stream. Likewise the directory forgets no list then: a stream list
        that comes back empty (Whatnot unreachable, every feed failing) used to leave only the camped streams' lists,
        and every other stream counted as not read yet until about 600 lists were fetched again (about 2 minutes)."""
        ids = self.interesting_show_ids()
        if ids:
            self.giveaways.refresh(ids, forget=bool(self.shows))
        if self.shows and time.time() - getattr(self, "_retargeted_at", 0.0) >= 60:
    ...
    def trial_settings(self) -> dict:
        """What the 1.2.0 trial runs under (givvy/trial.py snapshot): the accounts with a device ticked and the switches
        it is judged by. Stored with the trial's start, by the first Start and by Settings > Results."""
        return trial.snapshot(self.cfg, [r.account for r in self.runners if any(r.enabled.values())])
    ...
    def start(self):
        ...
        for r in self.runners:
            ...
            r.thread.start()
        # The first Start on this version starts the trial window, once ([trial] auto_mark): the report counts the
        # trial from here and the baseline before it, from the same state.db.
        try:
            now, label = time.time(), version.short_version()
            if trial.mark_start(self.store, self.cfg, now, label=label, settings=self.trial_settings()):
                self.note(f"trial window starts {time.strftime('%Y-%m-%d %H:%M', time.localtime(now))} ({label})")
        except Exception:
            log.exception("could not mark the start of the trial")
    ...
    def stop(self):
        ...
        for r in self.runners:
            ...
            r.out_since = 0.0
        for a in self.alerts.active():                   # a Stop is intentional: out of a stream is no news
            if a.key.startswith("out:"):
                self.alerts.resolve(a.key)

    WATCH_S = 10               # how often _watch_accounts looks (it only reads state)
    IN_STREAM = (Stage.WATCHING, Stage.ENTERING, Stage.CONFIRMING, Stage.HOLDING)

    def _watch_accounts(self, now: float) -> None:
        """The catch-all alert: an enabled account you did not pause that has not been in a stream, or whose loop has
        not ticked, for alerts.out_of_stream_min, whatever the cause. It also catches a camper loop that stalled, which
        the STALLED label only shows on screen. One alert per incident; a boot, safety-pause or sign-in alert for the
        same account says it better, so none is raised while one of those is active."""
        limit = self.cfg.alerts.out_of_stream_min * 60
        for r in self.runners:
            key, st = "out:" + r.account, r.camper.state
            in_stream = st.stage in self.IN_STREAM and now - r.last_tick < limit
            if (limit <= 0 or in_stream or (st.paused and st.paused_reason == "")
                    or not any(r.enabled.get(d.name, True) for d in r.devices)):
                r.out_since = 0.0
                # Said only when it is back in a stream: paused or unticked by you needs no notification.
                self.alerts.resolve(key, f"{r.account} is back in a stream" if in_stream else "")
                continue
            stalled = st.stage in self.IN_STREAM              # in a stream, but its loop has not ticked
            if not r.out_since:
                r.out_since = min(now, r.last_tick) if stalled and r.last_tick else now
            if now - r.out_since >= limit and not self.alerts.active(r.account) and not self._boot_pending(r):
                mins = int((now - r.out_since) // 60)
                if stalled:
                    text = (f"{r.account}'s loop has not ticked for {mins} min ({st.stage}: {st.last_action}). "
                            f"If it stays stuck, restart the app.")
                else:
                    problem = " ".join(why for why, _fix in r.problems.values())
                    text = (f"{r.account} has not been in a stream for {mins} min ({st.stage}: {st.last_action}). "
                            f"{problem}").strip()
                self.alerts.fire(key, text, r.account)

    @staticmethod
    def _boot_pending(r: AccountRunner) -> bool:
        """One of the account's emulators is booting, or has failed fewer times in a row than the boot alert needs:
        that alert is at most one boot away and says it better. In the 9/24 pattern (every boot hanging to its 240 s
        timeout) both fired, the out-of-stream alert at +900-910 s and the boot alert at +915-920 s: two notifications,
        two alerts.log lines, two @-mentions. Nothing is held back with emulator_alert_after = 0 (no boot alert)."""
        if r.alert_after <= 0:
            return False
        return any(r.is_booting(d.name) or 0 < r._fails.get(d.name, 0) < r.alert_after for d in r.devices)
    ...
    def _camp_loop(self, r: AccountRunner):
        ...
        while not self._stop.is_set():
            ...
            r.wake.clear()
        try:
            r.camper.end_turn()              # Stop or an update restart: the held giveaway keeps its peak count
        except Exception:
            log.exception("end_turn failed for %s", r.account)
        r.camper.end_stay()                  # and the stay's visits row ends now, not at the next start
        self.claims.release(r.account)
        ...
        try:
            while not self._stop.is_set():
                ...
                if self._refresh_asked.is_set() or now - last_disc >= self.cfg.whatnot.discovery_interval_s:
                    ...
                    last_disc = now
                if now - last_watch >= self.WATCH_S:
                    last_watch = now
                    try:
                        self._watch_accounts(time.time())     # after a discovery that took seconds
                    except Exception:
                        log.exception("account watchdog")
                self.last_tick = now
...
class App(tk.Tk):
    def __init__(self, autostart: bool = False):
        ...
        # The alert tray icon's 'Show Whatnot Givvy' runs on pystray's thread; Tk may only be touched from this one.
        self.engine.alerts.on_show = lambda: self.q.put(("call", self._show_window))
        self._banner_text = ""
    ...
    def _show_window(self):
        """The alert tray icon was clicked: bring the window back, minimised or not."""
        self.deiconify()
        self.lift()
    ...
    def _build(self):
        ...
        self.top_bar = top
        # Red, under the top bar, while an alert is active and not dismissed (packed by _tick). 2026-09-24 04:30-06:10
        # the only trace of both emulators being down was a line in givvy.log.
        self.alert_bar = ttk.Frame(self)
        self.alert_lbl = ttk.Label(self.alert_bar, text="", style="Alert.TLabel", wraplength=900, justify="left")
        self.alert_lbl.pack(side="left", fill="x", expand=True)
        ttk.Button(self.alert_bar, text="Dismiss", command=self.engine.alerts.dismiss).pack(side="right", padx=(8, 0))
    ...
    def _rename(self, device_name: str):
        ...
        hint = ("Letters, digits, - and _ (the account label may also have a '.'). Make the account label the "
                "account's exact Whatnot username: that is how its wins are recognised. It also tags the log and "
                "Discord lines and keeps the entry and win history."
    ...
    def _toggle_pause(self):
        """Pause, or Resume when every account is paused by you (paused_by_you). A safety or sign-in pause is the bot's
        own: pressing Pause turns it into your pause, and Resume stays the manual override."""
        paused = not paused_by_you(self.engine.runners)
        for r in self.engine.runners:
    ...
    def _tick(self):
        ...
        if held != self._last_held:                  # keep the 'held by' column honest
            ...
            self._fill_favs()
        want = "Resume" if paused_by_you(eng.runners) else "Pause"      # a pause of the bot's own reads 'Pause'
        ...
        for r_, box_ in self.mode_boxes:
            ...
            if box_.get() != want:
                box_.set(want)
                flipped = True
        if flipped and self._ui.get("modes") != eng.modes():
            self._save_modes()                       # a stay that ended went back to Scan: a restart must not undo it
        for runner, dev, var, (state_l, stream_l, cap_l, action_l) in self.rows:
            ...
            for b in self.power_btns.get(dev.name, ()):      # never two boot / power-off calls on one emulator
                b.config(state="disabled" if busy or runner.is_booting(dev.name) else "normal")
            ...
            if busy:
                ...
                paused = {"login": "NEEDS SIGN-IN", "breaker": "SAFETY PAUSE"}.get(st.paused_reason, "PAUSED")
                state = paused if st.paused else f"ACTIVE: {st.stage}"
            ...
            if active:
                ...
                nxt, fails = runner.next_try(dev.name), runner._fails.get(dev.name, 0)
                tries = (f"   ({fails} failed {'try' if fails == 1 else 'tries'}"
                         + (f", next try {time.strftime('%H:%M', time.localtime(nxt))}" if nxt else "") + ")"
                         if fails else "")
                stream_l.config(text=""); cap_l.config(text="")
                action_l.config(text=(why + tries + "   ->  Settings... > Emulator check")[:200])
            else:
    ...
    def _show_banner(self):
        """The red line under the top bar: the active alerts not dismissed (givvy/alerts.py), gone when there are none."""
        banner = self.engine.alerts.banner()
        if banner == self._banner_text:
            return
        self._banner_text = banner
        if banner:
            self.alert_lbl.config(text=banner)
            self.alert_bar.pack(fill="x", padx=10, after=self.top_bar)
        else:
    ...
    def _fill_tree(self, shows):
        ...
        for _lo, label in BANDS:
            ...
            for p in picks:
                s = p.show
                # score = expected wins/h if every giveaway were entered (givvy/crowd.py): about 0.01-0.6, so 3 decimals
                self.tree.insert("band:" + label, "end", iid=s.id,
                                 ...
                                 values=(s.game, s.viewers, eng.giveaway_counts.get(s.id, 0),
                                         p.excluded or f"{KIND_LABEL.get(p.kind, p.kind)}  {p.score:.3f}",
    ...
    def _quit(self):
        self.engine.stop()
        self.engine.alerts.close()                   # the alert tray icon goes with the window
        self.destroy()
...
def main():
    ...
    if "--check-reader" in sys.argv:
        ...
        lines = []
        try:
            devices = sorted(list_devices(cfg.device.android_sdk).items())
        except Exception as e:                   # adb itself failed (list_devices raises): say so in the file
            devices, lines = [], [f"adb failed: {e}"]
    ...
    if "--report" in sys.argv:
        ...
        print(text)
        sys.exit(0)
    if "--trial-report" in sys.argv:
        # The 1.2.0 trial against the version before (givvy/trial.py), as text: printed from source, and written to
        # trial_report.txt beside the exe; Settings > Results shows the same. It only reads: Store() adds nothing
        # beyond its CREATE IF NOT EXISTS, open stays count up to their last seen_at, and it is safe while the bot
        # runs (WAL).
        from .config import load as _load
        here = Path(sys.executable).resolve().parent if getattr(sys, "frozen", False) else Path.cwd()
        cfg = _load(here / "config.toml")
        store = Store(cfg.root / cfg.paths.db)
        text = trial.trial_report(store, cfg)
        (here / "trial_report.txt").write_text(text + "\n", encoding="utf-8")
        try:
            print(text)
        except UnicodeEncodeError:                  # a prize title with an emoji, printed to a cp1252 pipe
