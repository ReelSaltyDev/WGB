...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        self.theme_box = ttk.Combobox(foot, values=list(THEMES), state="readonly", width=15)
        ...
        self.blast_btn = ttk.Button(foot, text="Test the blast", command=self._test_blast)   # Time Bomb TCG only
    ...
    def _show_wins_today(self):
        ...
        if went_up and self.theme.celebrate:
            self._celebrate()
        if went_up:
    ...
    def _celebrate(self, caption: str | None = None):
        """Time Bomb TCG: the bomb drops, the fuse burns, it blows. '<account> Won <prize>' under it."""
        try:
    ...
    def _show_logo(self, show: bool):
        if show and getattr(self, "_logo_img", None) is None:
            try:
                self._logo_img = tk.PhotoImage(file=str(LOGO))
            except tk.TclError:
                log.warning("could not load %s", LOGO)
                show = False
        if show:
