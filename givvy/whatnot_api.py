...
Q_GET_FEED = (
    ...
    "{id activeViewers startTime status title livestreamCategories{id label deeplink} tags{id label} user{id username}}}}}}}}}")

Q_USER_LIVE = (
    "query GetUserLiveStreams($username:String!){getUser(username:$username){id username isLive "
    "livestreams(first:5){edges{node{id status title activeViewers startTime}}}}}")
