...
class EmulatorCheckTab(ttk.Frame):
    ...
    def _draw(self):
        ...
        for n, c in enumerate(self.checks):
            colour = {True: "#1a7f37", False: "#b42318", None: "#8a6d00"}[c.ok]
            ttk.Label(self.grid_frame, text=MARK[c.ok], foreground=colour, font=("Segoe UI", 11, "bold")
            ...
            ttk.Label(self.grid_frame, text=c.detail, wraplength=470, justify="left", foreground="#333"
        ...
        if not self.checks:
            return
        if first is None:
            self.summary.config(text="Everything an emulator needs is in place.", foreground="#1a7f37")
        else:
            need = "" if self.agree.get() or first.fix != "setup" else "  (tick the box above first)"
            self.summary.config(text=f"To fix first: {first.title}{need}", foreground="#b42318")
