...
class Store:
    ...
    def rename_account(self, old: str, new: str):
        """History follows the account when it is renamed in the window: entries, follows, and its wins, so the
        counter, the charts and the celebration name it by its new label (it used to leave the wins on the old one,
        and the charts split one account in two). What was learned about it too (its Whatnot name, givvy/whoami.py)."""
        ...
        for key in ("whatnot_name", "joins_seen", "label"):
            value = self.kv_get(f"{key}:{old}")
            if value is not None:
                self.kv_set(f"{key}:{new}", value)
                self._x("DELETE FROM kv WHERE key=?", (f"{key}:{old}",))
