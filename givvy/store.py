...
class Store:
    ...
    def record_entry(self, live_product_id: str, show: LiveShow, device: str, entries_seen: int | None,
                     result: str, at: float | None = None, account: str = "main"):
        self._x("INSERT INTO entries(live_product_id,show_id,seller,device,entered_at,entries_seen,result,account) "
                "VALUES(?,?,?,?,?,?,?,?)",
    ...
    def record_win(self, live_product_id: str, show: LiveShow, title: str):
        self._x("INSERT INTO wins(live_product_id,show_id,seller,title,detected_at) VALUES(?,?,?,?,?)",
                (live_product_id, show.id, show.seller, title, time.time()))
