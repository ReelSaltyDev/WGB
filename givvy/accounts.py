...
class AccountRunner:
    ...
    def __init__(self, account: str, devices: list, camper, cache_s: float = 5.0):
        ...
        self.wake = threading.Event()              # set when a giveaway starts in our stream: look NOW
        ...
        self._failed_at: dict[str, float] = {}     # device name -> when its last boot FAILED
        self._fails: dict[str, int] = {}           # device name -> failed boots in a row
        # Set by the engine from [device] emulator_retry_s / emulator_retry_max_s / emulator_alert_after (see
        # _retry_wait), and the engine's Alerts. These defaults are for tests.
        self.retry_s = 60
        self.retry_max_s = 1800
        self.alert_after = 3
        self.alerts = None
        self.boot_now = False                      # tests: boot on the calling thread
        self.last_tick = 0.0
        self.out_since = 0.0                       # the engine's watchdog: when this account was last seen out of a stream
    ...
    def power(self, device_name: str, action: str, wait: bool = False) -> None:
        ...
        Refused while it is busy or booting by itself (ensure_running; the window greys the buttons then too): two
        boot() or power_off() calls on one emulator share its process handle and log, and since 1.2.0 each can end the
        other's launch as a leftover or a stuck boot (Start during a hung 240 s boot: 3 launches, the hung one and then
        the fresh cold boot ended, both calls failed).
        """
        dev = next((d for d in self.devices if d.name == device_name), None)
        if dev is None or not hasattr(dev, "power_off") or device_name in self.busy or device_name in self._booting:
            return
        self.busy[device_name] = self.POWER[action]
        self._failed_at.pop(device_name, None)      # you asked: try now, whatever happened before
        self._fails.pop(device_name, None)          # ...and the waits start again from the shortest
            ...
            try:
                ...
                if action != "off":
                    ...
                    if self.alerts is not None:
                        self.alerts.resolve("boot:" + device_name)    # you turned it off: nothing left to warn of
            except Exception:
    ...
    def _retry_wait(self, n: int) -> float:
        """How long after its n-th failed boot in a row a device is left alone (or until you press Start): 60, 120,
        240, 480, 960 s, then every 30 min. It never gives up: the 9/22 outage ended by itself at 00:05, and a PC
        restart can fix what no retry does. It used to be a flat 300 s."""
        return min(self.retry_max_s, self.retry_s * 2 ** max(0, n - 1))

    def next_try(self, device_name: str) -> float | None:
        """When a failing device is booted again; None while it is not failing."""
        at = self._failed_at.get(device_name)
        return None if at is None else at + self._retry_wait(self._fails.get(device_name, 0))

    def _note_boot(self, dev, ok: bool, now: float | None = None) -> None:
        if ok:
            n = self._fails.pop(dev.name, 0)
            self.problems.pop(dev.name, None)
            self._failed_at.pop(dev.name, None)
            if self.alerts is not None:           # a no-op unless the alert fired; a Start you pressed resolves it too
                self.alerts.resolve("boot:" + dev.name, f"{dev.name} started again"
                                    + (f" after {n} failed tries" if n else ""))
        else:
            n = self._fails[dev.name] = self._fails.get(dev.name, 0) + 1
            why = getattr(dev, "last_error", "") or "The emulator did not start."
            self.problems[dev.name] = (why, getattr(dev, "last_fix", ""))
            self._failed_at[dev.name] = time.time() if now is None else now
            if n == self.alert_after and self.alerts is not None:
                # One alert per incident: an out-of-stream alert for this account is the same one, and this says it
                # better (the engine holds that one back while a boot is pending; this covers one that got out first).
                self.alerts.resolve("out:" + self.account)
                self.alerts.fire("boot:" + dev.name, f"{dev.name} could not start {n} times in a row: "
                                 f"{why.rstrip('.')}. Trying again every {self.retry_max_s // 60} min at most. If it "
                                 f"keeps failing, restart the PC.", account=self.account)
    ...
    def ensure_running(self, now: float | None = None):
        ...
        A boot that FAILS is remembered with its reason and left alone for a while (_retry_wait), counted from when
        it failed: 9/22 and 9/24 the retry came 60 s after a 240 s timeout, because the wait counted from the start.
        ...
        for d in self.devices:
            if not (self.enabled.get(d.name, True) and hasattr(d, "boot") and d.name not in self._booting
                ...
                continue
            if now - self._failed_at.get(d.name, -1e18) < self._retry_wait(self._fails.get(d.name, 0)):
            ...
            def run(dev=d, now=now):
                ...
                try:
                    ...
                    self._note_boot(dev, ok, now + (time.time() - t))     # when it FAILED, not when it started
...
def build_device(a: AccountCfg, cfg: Config, dry_run: bool = False):
    ...
    # The winner dialog reads '<username> won the giveaway!'. Seen live: a keychain win went unnoticed
    # because no whatnot_username was configured. The account label is usually the Whatnot name, so it
    # counts too; set whatnot_username when they differ.
    dev.usernames = {u.strip().lower() for u in (a.whatnot_username, a.account, a.name) if u and u.strip()}
    dev.cpu_affinity, dev.priority = a.cpu_affinity, a.priority
    dev.keep_screens = not dry_run
    import os
    if (getattr(cfg.device, "record", False) or os.environ.get("GIVVY_RECORD") == "1") and not dry_run:
        from pathlib import Path
        from .replay import Recorder
        dev.recorder = Recorder(Path.cwd() / "recordings", a.name)
    if getattr(cfg.device, "fast_reader", False) and not dry_run:
        from pathlib import Path
        from .device.reader import FastReader
        dev.reader = FastReader(dev.serial, str(Path(cfg.device.android_sdk) / "platform-tools" / "adb.exe"))
    return dev
...
ACCOUNT_RE = re.compile(r"[A-Za-z0-9_.-]{1,30}")     # an account label is usually the Whatnot name, which can have a '.'
...
def rename_in_config(path: Path | str, old_name: str, new_name: str, new_account: str) -> tuple[str, str]:
    ...
    if not NAME_RE.fullmatch(new_name or ""):
        raise ValueError("device name: 1-24 letters, digits, - or _")
    if not ACCOUNT_RE.fullmatch(new_account or ""):
        raise ValueError("account label: 1-30 letters, digits, ., - or _")
