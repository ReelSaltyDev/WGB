...
class Stage:
    ...
    OFF = "off"                      # no device, paused, waiting for a sign-in, or you are using the phone
...
class Camper:
    ...
    def _enter_here(self, device, s, now: float, key: str, show, ns=None):
        ...
        fast = False
        try:
            if ns is not None and hasattr(device, "fast_enter") and self._fast_ok(show, s, now):
            ...
            for attempt in range(MAX_EXPANDS):
                ...
                if verdict == "buyers":
                    ...
                    self.store.record_entry(key, show, device.name, s.entries, "skipped", account=self.account, prize=s.prize)
                    self._card_handled = True
                    return
                if verdict == "leave":
            ...
            if tapped is not None:
                ...
                self.store.record_entry(key, show, device.name, s.entries, "missed", account=self.account, prize=s.prize)
            self._card_handled = True
        except Exception as e:
    ...
    def _ask_the_tick(self, device, show, ns, s, enter_xy, now: float) -> None:
        ...
        try:
            ...
            verdict = None
        if verdict is True:
            self._confirm_late(device, show, s, now, how="confirmed by the tick on the badge")
    ...
    def _confirm_late(self, device, show, s, now: float,
                      how: str = "confirmed late: the card was hidden when we first looked") -> bool:
        p = getattr(self, "_pending", None)
        ...
        self.store.confirm_entry(p["key"], s.prize)
        st.entered_here += 1
        self._card_handled = True
    ...
    def _upgrade(self, now: float) -> bool:
        ...
        st = self.state
        camped = st.camped
        if camped is None or now - self.visit.upgrade_checked < self.UPGRADE_CHECK_S:
            return False
        self.visit.upgrade_checked = now
