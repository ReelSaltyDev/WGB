...
def rank_streams(shows: list[LiveShow], giveaway_counts: dict[str, int], cfg: Config,
    ...
    for s in shows:
        ...
        tier = (0 if buyers_only else 1, 0 if s.id in cooling else 1, known,
                0 if n_buyers else 1, 1 if has_pack else 0, -s.viewers)
...
class Camper:
    ...
    def _new_giveaway_behind_the_same_badge(self, s, now: float) -> bool:
        ...
        giveaway the entry count only climbs; when it DROPS, a new one has started. And nothing stays
        'handled' beyond hold_timeout_s: look again (an entry we already have just reads 'already entered')."""
        if s.entries is not None:
            if self._handled_max is not None and s.entries < self._handled_max:
