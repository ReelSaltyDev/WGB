...
def rank_streams(shows: list[LiveShow], giveaway_counts: dict[str, int], cfg: Config,
    ...
    for s in shows:
        ...
        tier = (0 if buyers_only else 1, 0 if s.id in cooling else 1, known,
                1 if (info is not None and info.pack) else 0, -s.viewers)
        out.append(StreamPick(s, total, ("pack listed, " if tier[3] else "") + f"{s.viewers} viewers | "
