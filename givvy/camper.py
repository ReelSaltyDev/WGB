...
def rank_streams(shows: list[LiveShow], giveaway_counts: dict[str, int], cfg: Config,
                 ...
                 cooling: set[str] | frozenset = frozenset(), keep_excluded: bool = False) -> list[StreamPick]:
    ...
    for s in shows:
        ...
        info = (listed or {}).get(s.id)
        if isinstance(info, int):                      # older callers pass a plain count
            from .giveaway_info import Listed
            info = Listed(open=info, pack=False, buyers=0)
        n_listed = None if info is None else info.open
        ...
        # THE OWNER'S ORDER, as strict priorities rather than points:
        #   1. not a buyers-only stream (absolute: it is dropped, whatever else it has going for it)
        #   2. 'pack' in the description of a giveaway anyone may enter
        #   3. fewest viewers = best odds
        # Known-to-list-something beats not-looked-up-yet beats known-to-list-nothing, and a stream just
        # left for being dead goes to the back, or the smallest stream would be picked again for ever.
        # The old point score only breaks ties.
        buyers_only = info is not None and info.open == 0 and info.buyers > 0
        if buyers_only and not keep_excluded:
            continue
        # 'Focus on packs': every open giveaway here is a card or a sticker. Not joined (a stream that ALSO
        # gives packs is kept; its card giveaways are skipped on the card itself).
        lesser_only = (cfg.scoring.skip_cards_and_stickers and info is not None and info.open > 0
                       and info.grade == 0)
        if lesser_only and not keep_excluded:
            continue
        known = 1 if info is None else (2 if info.open > 0 else 0)
        has_pack = info is not None and info.pack
        # 2 a pack, 1 something else, 0 'card'/'sticker' in the description (chosen last, still entered)
        grade = 0 if info is None or info.open == 0 else (info.grade if info.grade >= 0 else (2 if info.pack else 1))
        n_buyers = 0 if info is None else info.buyers
        # Wanted prize first, then fewest viewers. 'No buyers giveaway here' only breaks ties: it used to come
        # before the prize, and sent an account past the pack streams to a clean stream giving keychains.
        # (Buyers giveaways themselves are never entered, and a buyers-ONLY stream is dropped above.)
        tier = (0 if (buyers_only or lesser_only) else 1, 0 if s.id in cooling else 1, known,
                grade, -s.viewers, 0 if n_buyers else 1)
        out.append(StreamPick(s, total, ("pack listed, " if has_pack else "")
                              + ("unwanted prizes, " if (info is not None and info.open and grade == 0) else "")
                              + (f"{n_buyers} buyers giveaway(s) listed, " if n_buyers else "") + f"{s.viewers} viewers | "
                              f"game {game_w:.0f} + activity {activity:.0f} "
                              ...
                              tier=tier, excluded="buyers only" if buyers_only else ("unwanted prizes only" if lesser_only else ""),
                              pack=has_pack, buyers=n_buyers))
    out.sort(key=lambda p: (p.tier, p.score, GAME_ORDER.get(p.show.game, 0)), reverse=True)
    return out
...
class Camper:
    ...
    def _enter_here(self, device, s, now: float, key: str, show, ns=None):
        ...
        try:
            for attempt in range(MAX_EXPANDS):
                ...
                flagged = False
                if self.buyers_title_lookup is not None and s.prize:
                    try:
                        flagged = bool(self.buyers_title_lookup(show.id, s.prize))
                    except Exception:
                        flagged = False
                if flagged or says_buyers(s.prize):
                    # Absolute rule. The listing can be missing or wrong; the card's own words are not.
                    ...
                    self._card_handled = True
                    return
                from .giveaway_info import grade_title
                if self.cfg.scoring.skip_cards_and_stickers and s.prize and grade_title(s.prize) == 0:
                    st.note(f"skipped {s.prize}: not a wanted prize (card, sticker, keychain...)")
                    self.store.record_entry(key, show, device.name, s.entries, "skipped", account=self.account)
                    self._card_handled = True
                    return
    ...
    def _maybe_move_on(self, now: float):
        """After a quiet spell: Scan mode considers a better stream, 'stay in one stream' leaves a dead one."""
        st = self.state
        if st.mode == "single":
        ...
        if camped is None:
            return
        # The ranking is strict (pack, then fewest viewers), so the stream we sit in is usually
        # still 'the best' while it is dead. Rank the OTHERS and decide separately whether to go.
        others = [p for p in ranked if p.show.id != camped.id]
        if not others:
            self._idle_since = now
            return
        best = others[0]
        here = next((p for p in ranked if p.show.id == camped.id), None)
        dead = not st.giveaways_seen_here or quiet_for >= 3 * limit
        if not dead and here is not None and here.tier >= best.tier:
            # It has run giveaways for us and nothing ranks above it: between giveaways, not dead.
            return
        st.note(f"quiet for {quiet_for:.0f}s; moving to {best.show.seller}")
        if dead:
            # Without this the scanner picked the same dead stream again minutes later:
            # five times in a row, seen live.
