...
def rank_streams(shows: list[LiveShow], giveaway_counts: dict[str, int], cfg: Config,
    ...
    for s in shows:
        ...
        # 'Does not say buyers' is the FIRST priority: a stream with no buyers giveaway at all comes before
        # a mixed one (seen live: 8 of 13 streams joined in a morning were mixed), which still beats nothing.
        tier = (0 if buyers_only else 1, 0 if s.id in cooling else 1, known,
                0 if n_buyers else 1, grade, -s.viewers)
        out.append(StreamPick(s, total, ("pack listed, " if has_pack else "")
                              + ("card/sticker giveaways, " if (info is not None and info.open and grade == 0) else "")
                              ...
                              tier=tier, excluded="buyers only" if buyers_only else "",
