...
def draw_chart(cv: tk.Canvas, kind: str, title: str, subtitle: str, x_title: str, y_title: str,
               ...
               legend: bool = False):
    """One chart: 'column' or 'line'; series = [(name, values, colour)]."""
    cv.delete("all")
    w, h = max(cv.winfo_width(), 240), max(cv.winfo_height(), 160)
    cv.create_text(16, 16, text=title, anchor="nw", font=(FONT, 12), fill=INK)
    top = 44
    if subtitle:
...
class WinChartsPanel(ttk.LabelFrame):
    ...
    def __init__(self, master, store, accounts=lambda: ()):
        ...
        self.note = ttk.Label(self, text=self.NOTE, foreground="#666", font=(FONT, 8), wraplength=460,
                              justify="left")
    ...
    def redraw(self):
        ...
        names = {a: self.store.kv_get(f"label:{a}") or a for a in accounts_in(rows, known)}
        colors = {a: COLORS[i % len(COLORS)] for i, a in enumerate(accounts_in(rows, known))}
        blue = COLORS[0]
        ...
        for key, kind, t, x_title, y_title, _h in CHARTS[1:]:
            ...
            draw_chart(c[key], kind, t, "", x_title, y_title, labels, series, legend=key == "by_acct")
