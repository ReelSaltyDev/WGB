...
def draw_chart(cv: tk.Canvas, kind: str, title: str, subtitle: str, x_title: str, y_title: str,
    ...
    # ~44 px per date, as Sheets thins them out; a chart with a number on every bar dates every bar when it can
    every = max(1, -(-n * (28 if data_labels else 44) // max(1, int(right - left))))
    ...
    if kind == "column":
        ...
        for i, v in enumerate(vals):
            ...
            if data_labels:                                    # smaller when the days are packed in
                cv.create_text(xs[i], y_of(v) - 9, text=str(v), font=(FONT, 9 if slot >= 24 else 7), fill=INK)
    else:
    ...
    def redraw(self):
        ...
        for key, kind, t, x_title, y_title, _h in CHARTS[1:]:
            ...
            if key == "by_acct":
                ...
                # John, 2026-09-25: newest day first, and each day's number on its bar
                days, series = labels[::-1], [(y_title, totals[::-1], blue)]
            else:
                series = [(y_title, totals, blue)]
            draw_chart(c[key], kind, t, "", x_title, y_title, days, series, data_labels=key == "count",
                       legend=key == "by_acct", pal=pal)
