...
class Theme:
    ...
    caption_text: str = ""
    border: str = ""
    logo: str = ""              # beside Total Wins Today: a file in givvy/assets, or in the theme's own folder
    celebration: str = ""       # on every win: 'bomb' (givvy/blast.py) or 'snowball' (givvy/snowball.py)
    assets: str = ""            # a folder under themes/ this theme needs; without it the theme is not offered
    credit: str = ""            # shown in the bottom bar while the theme is on (art that is someone else's)
    # Win Charts
    chart_bg: str = "white"
...
# Riot's fan-content policy ("Legal Jibber Jabber") asks free fan projects that use its art to say this.
RIOT = ("Nunu & Willump art © Riot Games. Whatnot Givvy isn't endorsed by Riot Games and doesn't reflect the "
        "views or opinions of Riot Games or anyone officially involved in producing or managing Riot Games properties.")
...
THEMES: dict[str, Theme] = {t.name: t for t in (
    ...
    Theme("Time Bomb TCG", base="clam",
          ...
          caption="#0c1530", caption_text="#f3eddc", border="#e5b544", logo="timebomb-logo.png",
          celebration="bomb",
    ...
    # League of Legends' Nunu & Willump (Nunu Bot): Freljord ice with Nunu Bot's orange. Its art is Riot's, rendered
    # from the game's own files into themes/nunu/; it ships in the installer but is never in the source code.
    Theme("Nunu & Willump", base="clam",
          bg="#0a1726", surface="#0f2238", surface2="#143052", line="#23466e", text="#eaf6ff", muted="#9ec4e4",
          faint="#6f93b5", accent="#8fd8ff", button="#143052", button_hover="#1d4270", select="#2f8fd8",
          select_text="#ffffff", good="#6fe0a8", warn="#ffd27a", bad="#ff8a80", today="#bfeaff",
          today_flash="#ffffff", heading_font="Segoe UI Black", dark_title_bar=True,
          caption="#0f2238", caption_text="#eaf6ff", border="#8fd8ff", logo="logo.png", celebration="snowball",
          assets="nunu", credit=RIOT,
          chart_bg="#0f2238", chart_panel="#0a1726", chart_edge="#23466e", chart_ink="#eaf6ff",
          chart_muted="#9ec4e4", chart_grid="#1c3a5c", chart_axis="#2d5680", chart_bars="#8fd8ff",
          chart_series=("#8fd8ff", "#ff9f43", "#c9b8ff", "#6fe0a8", "#ffd27a", "#5fa8ff", "#ff8a80", "#9ec4e4")),
)}
BUNDLED = Path(__file__).resolve().parent / "assets"
THEME_DIRS: list[Path] = []     # where themes/<name>/ folders are looked for; the app adds <its folder>/themes
DEFAULT = "Light"
_native: dict = {}              # the platform theme's name, remembered before anything is changed


def asset_dir(t: Theme) -> Path | None:
    """The theme's own folder (themes/<assets>/), or None when this PC does not have it."""
    if not t.assets:
        return None
    for base in THEME_DIRS:
        d = Path(base) / t.assets
        if (d / "roll_00.png").exists() or (d / "logo.png").exists():
            return d
    return None


def available() -> list[str]:
    """The themes this PC can show: those needing art only when the art is here."""
    return [n for n, t in THEMES.items() if not t.assets or asset_dir(t) is not None]


def logo_path(t: Theme) -> Path | None:
    if not t.logo:
        return None
    d = asset_dir(t) if t.assets else BUNDLED
    p = (d / t.logo) if d else None
    return p if p is not None and p.exists() else None


def get(name: str) -> Theme:
    t = THEMES.get(name)
    return t if t is not None and name in available() else THEMES[DEFAULT]
...
def _style_labels(style: ttk.Style, t: Theme):
    ...
    style.configure("Alert.TLabel", foreground=t.bad, background=bg, font=(UI_FONT, 10, "bold"))   # the alert banner
