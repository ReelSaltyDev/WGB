...
def personal_strings() -> list[str]:
    ...
    vals = [d.get("bot_token"), d.get("channel_id"), d.get("alerts_channel_id"), d.get("mention", "").strip("<@>"),
            dev.get("phone_serial")]
...
def main(app_id_suffix: str = "") -> Path:
    ...
    try:
        pyinstaller("--windowed", "--name", "WhatnotGivvy", "--distpath", str(DIST), "--icon", str(icon),
                    ...
                    "--hidden-import", "givvy.themes", "--hidden-import", "givvy.blast", "--hidden-import", "givvy.snowball",
                    ...
                    # alerts: without these the installed build silently has no Windows notification (pystray picks its
                    # backend inside a function, so PyInstaller does not see _win32)
                    "--hidden-import", "givvy.alerts", "--hidden-import", "givvy.tray",
    ...
    # Theme art that is not in the source code (themes/ is git-ignored): Nunu & Willump is Riot's, rendered from the
    # game on this PC. John's call (2026-09-24): it ships in the installer so his friend's update just works; it is
    # still never pushed with the source.
    art = ROOT / "themes"
    if art.is_dir() and any(art.iterdir()):
        shutil.copytree(art, payload / "themes", dirs_exist_ok=True)
        print("theme art:", ", ".join(sorted(p.name for p in art.iterdir() if p.is_dir())))
    else:
        print("no theme art on this PC: themes that need it will not be offered")
    audit(payload)
